...
class GraphConvConstants(object):
  ...
  EXTENDED_FEATURE_GENERATORS: Dict[str, MolecularFeaturizer] ={
...
def generate_global_features(mol: RDKitMol,
                             features_generators: List[str], **kwargs) -> np.ndarray:
  """
  ...
  extended_generators = GraphConvConstants.EXTENDED_FEATURE_GENERATORS
  ...
  for generator in features_generators:
    ...
    elif generator in extended_generators:
        if generator in kwargs:
          try:
            global_features.append(float(kwargs[generator]))
          except ValueError:
            logger.warning(f"{generator} cannot convert to float. Ensure that you are only using numeric values")
...
class DMPNNFeaturizer_Custom(MolecularFeaturizer):
  """
...
# featurise = DMPNNFeaturizer(features_generators=['morgan','temperature','rdkit_desc_normalized'])
